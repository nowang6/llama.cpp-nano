...
项目分为两个子目录：
- **`model_converter/`** — 模型导出、格式转换（HF→GGUF）、量化
- **`inference/`** — 推理引擎（C++ 编译运行）
...
完整类型列表可通过 `./build/bin/llama-quantize` 查看。
...
```
model_converter/          # 模型转换与量化
├── convert_hf_to_gguf.py # HF → GGUF 主脚本
├── conversion/           # 模型架构转换实现（Python）
├── gguf-py/              # GGUF 格式读写库
├── tools/quantize/       # C++ 量化工具
├── Qwen3-0.6B/           # 源 HuggingFace 模型
├── CMakeLists.txt        # 量化工具构建配置
└── pyproject.toml        # Python 项目配置

inference/               # 推理引擎
├── CMakeLists.txt        # 推理构建配置
├── main.cpp              # 推理 CLI（~200 行）
├── include/llama.h       # 公开 C API 头文件
├── src/                  # LLaMA 模型加载/推理/采样
├── ggml/                 # 张量计算库（仅 CPU 后端）
└── cmake/                # CMake 模块
